...
const Ggame = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen w-screen bg-gradient-to-r from-purple-500 to-blue-500 p-6">
      ...
      {gameOver ? (
        ...
        <div
          className={`grid gap-4 p-4 bg-white rounded-lg shadow-lg w-full max-w-4xl ${gridSize === 6 ? "grid-cols-2 sm:grid-cols-3" : "grid-cols-3 sm:grid-cols-4"}`}
        >
          {Array.from({ length: gridSize }).map((_, index) => (
            <button
              key={index}
              onClick={() => handleBoxClick(index)}
              className="w-full h-24 sm:h-32 bg-gray-300 text-black font-bold rounded-lg flex items-center justify-center hover:bg-gray-400"
            >
